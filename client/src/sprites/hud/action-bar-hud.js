...
class ActionBarHud extends Container {
  constructor(app, tooltipContainer = null) {
    ...
    this.slotConfigs = Array(SLOT_COUNT)
      .fill(null)
      .map(() => ({
        abilityId: null,
        modifiers: [],
  ...
  renderBackground() {
    ...
    this.bg = new Graphics().rect(0, 0, width, height).fill(HUD_FILL_COLOR)
  ...
  updateUnlockedAbilities(playerStats) {
    ...
    for (const slot of this.slots) {
      slot.updateUnlockedState(this.getUnlockedAbilities(playerStats), this.getUnlockedModifiers(playerStats))
    }
