...
func _on_message_received(message: Dictionary) -> void:
	...
	match msg_type:
		...
		"respawn_success":
			_handle_respawn_success(message)
...
func _spawn_local_player(player_id: String) -> void:
	...
	if _initial_spawn_position == Vector3.ZERO:
		_initial_spawn_position = local_player.global_position
...
func _handle_world_state(state: Dictionary) -> void:
	...
	for enemy_data in enemies_data:
		...
		if not enemies.has(eid):
			...
			# Setup visuals AFTER enemy_type is set
			enemy.setup_visuals()

			print("[WORLD] Created new enemy: ", eid, " type: ", enemy.enemy_type, " mesh: ", enemy.mesh_instance != null, " rigged: ", enemy.rigged_model != null)

			# Register with UI manager (DISABLED - using 3D health bars now)
			# if enemy_ui_manager:
		...
		if is_instance_valid(enemies[eid]):
			...
			# Update UI with health (DISABLED - using 3D health bars now)
			# if enemy_ui_manager:
	...
	for eid in enemies.keys():
		if not eid in current_enemy_ids:
			...
			# Unregister from UI manager (DISABLED - using 3D health bars now)
			# if enemy_ui_manager:
...
func _handle_death_event(death_event: Dictionary) -> void:
	...
	# Remove health bar immediately on death (DISABLED - using 3D health bars now)
...
func _update_minion_state(minion: Node3D, state: Dictionary) -> void:
	...
	if not minion.has_meta("first_position_set"):
		minion.global_position = target_position
		minion.set_meta("first_position_set", true)
	else:
...
func _setup_death_screen() -> void:
	## Create death screen UI (hidden by default)
	...
	_get_hud_parent().add_child(death_screen)
	...
	death_label.add_theme_color_override("font_outline_color", Color(0, 0, 0))
	...
	death_label.position = Vector2(-100, -100)
	death_label.size = Vector2(200, 50)
	...
	respawn_button.set_anchors_preset(Control.PRESET_CENTER)
	respawn_button.position = Vector2(-100, 0)
	respawn_button.size = Vector2(200, 50)
	respawn_button.pressed.connect(_on_respawn_button_pressed)
	death_screen.add_child(respawn_button)

func _check_player_death() -> void:
	## Check if player health reached zero and show death screen
	if is_instance_valid(local_player) and not _is_player_dead:
		if _player_current_health <= 0:
...
func _on_respawn_button_pressed() -> void:
	## Handle respawn button click - request respawn from server
	print("[WORLD] Requesting respawn...")

	# Send respawn request to server
	var respawn_message = {
		"type": "respawn",
		"playerID": GameManager.local_player_id
	}
	NetworkManager.send_message(respawn_message)

func _handle_respawn_success(message: Dictionary) -> void:
	## Handle respawn success from server
	print("[WORLD] Respawn successful!")
	...
	# Reset player health
	_player_current_health = _player_max_health

	# Reset mouse mode to allow movement
