...
export const SampleItems = [...chestItems, ...handsItems, ...feetItems, ...headItems, ...weaponItems, ...bonusItems]
...
export const generateRandomItemOfType = itemTypes => {
  ...
  if (possibleItemsOfType.length === 0) {
    throw new Error(`cannot generate random item, no items of types: ${itemTypes.map(it => it.name).join(', ')}`)
  }
  return stampItemId(structuredClone(possibleItemsOfType[Math.floor(Math.random() * possibleItemsOfType.length)]))
}
