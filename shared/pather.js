...
class Pather {
  constructor(level) {
    ...
    const width =
      this.level.tileGrid.reduce((max, row) => Math.max(max, row.length), 0) *
      10
  ...
  findPath(from, to) {
    ...
    const lineBetween = Util.interpolate(fromX, fromY, toX, toY).filter(
      ([x, y]) => this.grid.isWalkableAt(x, y)
    )
    let path = null
